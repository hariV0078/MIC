...
app = FastAPI(
    ...
    description="Event Validation System with Ollama Open Source LLM (Parallel Processing) - FastAPI",
...
def get_config() -> ValidationConfig:
    ...
    if _config is None:
        ...
        # Ollama configuration is handled in get_ollama_client()
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "ollama_configured": ollama_client.client is not None,
        "ollama_base_url": ollama_client.base_url,
        "ollama_text_model": ollama_client.text_model,
        "ollama_vision_model": ollama_client.vision_model,
...
async def validate_batch(
    ...
    ollama_base_url: Optional[str] = Query(None),
        ...
        if use_sequential:
            ...
            for i, row in enumerate(submissions):
                try:
                    ...
                    # Rate limiter will handle delays automatically in ollama_client
                    
                    submission = process_submission(row, config, ollama_client)
            ...
            def process_single_submission(row_data: dict, row_index: int) -> tuple[int, dict]:
                ...
                # Note: The rate limiter in ollama_client will handle actual API rate limiting
                with _api_semaphore:
                    try:
