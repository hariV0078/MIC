...
def print_banner():
    ...
    |                                                               |
    |          Powered by Ollama Open Source LLM                      |
    |                                                               |
    +---------------------------------------------------------------+
    """
...
Examples:
  ...
  python main.py --non-interactive input.csv --output-csv output.csv
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Ollama vision model (default: llava:latest)'
    )
    parser.add_argument(
    ...
    if not args.non_interactive:
        ...
        print(Colors.BRIGHT_WHITE + "  Ollama URL:  " + Colors.RESET + ollama_base_url)
        print(Colors.BRIGHT_WHITE + "  Text Model:  " + Colors.RESET + ollama_text_model)
        print(Colors.BRIGHT_WHITE + "  Vision Model:" + Colors.RESET + ollama_vision_model)
        ...
        ollama_text_model = args.ollama_text_model or os.getenv('OLLAMA_TEXT_MODEL', 'llama3.2:3b')
        ollama_vision_model = args.ollama_vision_model or os.getenv('OLLAMA_VISION_MODEL', 'llava:latest')
    ...
    config = ValidationConfig(
        ...
        groq_api_key=None  # Not used in Ollama version
    )
    ...
    logger.info(f"Ollama Text Model: {ollama_text_model}")
    logger.info(f"Ollama Vision Model: {ollama_vision_model}")
    logger.info("=" * 60)
    
    # Process CSV
    try:
        process_csv(input_csv, output_csv, config, ollama_base_url=ollama_base_url, ollama_text_model=ollama_text_model, ollama_vision_model=ollama_vision_model)
        logger.info("=" * 60)
