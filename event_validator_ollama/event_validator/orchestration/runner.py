...
def _add_score_breakdown_to_row(enriched_row: dict, all_results: List[ValidationResult]) -> dict:
    ...
    for result in all_results:
        ...
        # Format: "2.0/2" or "0.0/7"
        enriched_row[column_name] = f"{float(result.points_awarded)}/{max_points}"
...
def process_submission(
    ...
    if not budget.can_make_call("theme_alignment"):
        ...
        theme_results = validate_theme(submission, ollama_client)
    ...
    if event_driven == 3 and theme_alignment_failed:
        ...
        if not budget.can_make_call("pdf_validation"):
            ...
            pdf_results = validate_pdf(submission, ollama_client)
    ...
    if submission.kill_switch:
        ...
        from event_validator.config.rules import THEME_RULES, IMAGE_RULES, SIMILARITY_RULES
        ...
        if submission.images:
            ...
            if not budget.can_make_call("image_validation"):
                ...
                image_results = validate_images(submission, ollama_client)
    ...
    if failed_results:
        ...
        CRITERION_CATEGORY_MAP = {
            # Theme rules
            # Theme rules
            "Title matches theme": "Theme",
            "Level matches duration": "Title",
            "Participants reported > 15": "Theme",
            # PDF rules
            "PDF title matches metadata": "PDF",
            "Expert details present": "PDF",
            "Objectives and learning align": "PDF",
            # Image rules
            ...
            # Similarity rules
            "Duplicate image check": "Image Analysis",
            "Duplicate title check": "Title",
        }
        
        # Standardized failure messages (fixed, deterministic)
        CRITERION_FAILURE_MSG = {
            "Title matches theme": "Title/Objectives/Learning do not align with declared theme",
            ...
            "Objectives and learning align": "Objectives/Learning outcomes not clearly stated/aligned in PDF",
            ...
            "Duplicate image check": "Duplicate or similar images detected",
            "Duplicate title check": "Duplicate title submitted by same user",
        }
        ...
        def format_failure_message(result: ValidationResult) -> str:
            ...
            # This fixes the "Deterministic Comment Bug" while keeping category grouping
            if result.message and len(result.message) > 5:
                # If it's the standard rejection message from pdf_validator, it's already good
                return result.message
            
            # Fallback to standardized message from map if validator message is generic
...
def process_csv(
    ...
    ollama_text_model = ollama_text_model or os.getenv('OLLAMA_TEXT_MODEL', 'llama3.2:3b')
    ollama_vision_model = ollama_vision_model or os.getenv('OLLAMA_VISION_MODEL', 'llava:latest')
    ollama_client = OllamaClient(
    ...
    if not ollama_client.client:
        logger.warning("Ollama client not initialized. Make sure Ollama is running: ollama serve")
    ...
    def process_single_row(row_data: dict, index: int) -> tuple[int, dict]:
        ...
        try:
            ...
            score_columns = [
                'themeAlignmentScore', 'levelDurationScore',
                'pdfTitleScore', 'pdfExpertScore', 'pdfAlignmentScore',
                'imgGeotagScore', 'imgBannerScore', 'imgRealActivityScore', 'imgParticipantsScore',
                'duplicateImageScore', 'duplicateTitleScore'
            ]
    ...
    # Prepare output CSV with score columns
    score_columns = [
        'themeAlignmentScore', 'levelDurationScore', 'themeParticipantsScore',
        'pdfTitleScore', 'pdfExpertScore', 'pdfAlignmentScore',
        'imgGeotagScore', 'imgBannerScore', 'imgRealActivityScore', 'imgParticipantsScore',
